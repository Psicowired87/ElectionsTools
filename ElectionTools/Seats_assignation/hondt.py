...
class Hondt_assignation(Seat_assignator):
    ...
    def assignation(self, votes, cutoff=0):
        ...
        seats, prices = transform_votes2seats_hondt(votes, self.votetypes,
                                                    self.n_seats, cutoff)
        return seats, prices

    def compute_unused_votes(self, seatable_votes, seats, prices, cutoff=0):
...
def hondt_method(votes, n_seats, cutoff=0):
    """Hondt method to assign seats from votes.
    ...
    price = np.max(computable_votes/float(seats))
...
def transform_votes2seats_hondt(votes, votetypes, n_seats, cutoff=0):
    ...
    for i in range(n):
        seats[i, :], prices[i] = hondt_method(seatable_votes[i, :], n_seats[i],
                                              cutoff[i])
